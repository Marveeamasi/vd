...
export default function SelectList({ lists, searchable, handleClick, setShowList }) {
  ...
  return (
    <div 
        className="w-[122px] shadow-lg py-[10px] gap-[5px] transition-all duration-300 ease-in-out px-[5px] rounded-[8px] bg-[#261148] absolute top-[110%] left-0 flex flex-col items-center justify-center z-30"
    ...
    >
        ...
        {filteredlists.map((option) => (
            <div
                onClick={(e) => {
                    e.stopPropagation();
                    handleClick(option); // Update selected option
                    setShowList(false);
                }}
                key={option}
                value={option}
